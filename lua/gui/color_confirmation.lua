...
local confirm_recruitlist = function (recruitlist)
	...
	local images = pyr_npt.helper.tablemap(recruitlist, function(uid) return wesnoth.unit_types[uid].__cfg.image or pyr_npt.helper.thex_png end)
	...
	for row = 1, rows do
		...
		for col = 1, image_per_row do
			...
			local image = pyr_npt.helper.thex_png
	...
	return gui.show_dialog(dialog_wml) == 1
end
