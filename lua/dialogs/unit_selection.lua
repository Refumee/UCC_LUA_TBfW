...
function unit_selection.get_unit_types_current_era_ids(leaders)
	local era = wesnoth.scenario.era
	era = era or {
		T.multiplayer_side {
	...
	for multiplayer_side in wml.child_range(era, "multiplayer_side") do
		if multiplayer_side.recruit ~= nil then
			for word in string.gmatch(multiplayer_side.recruit, '([^,]+)') do
				units[pyr_npt.helper.trim(word)] = true
			end
		end
...
function unit_selection.get_unit_types_current_era_and_advancemts()
	...
	local r = {}
	local known_units = pyr_npt.helper.deepcopy(units_to_do)
		...
		for k,v in pairs(units_to_do) do
			...
			if cfg ~= nil then
				...
				for word in string.gmatch(cfg.advances_to or "", '([^,]+)') do
					local unitid = pyr_npt.helper.trim(word)
...
function unit_selection.get_unit_types(side)
	local pool_type = tostring(wml.variables.pyr_npt_unit_pool_type or "recruitable")
...
function unit_selection.random_choice(unittypes, max_gold, max_count)
	...
	while units_left > 0 do
		...
		local max_cost = pyr_npt.helper.min(gold_left, (1.7 * gold_left / units_left))
...
function unit_selection.fill_random(chosen_units, all_unittypes, max_gold, max_count)
	...
	for k, u in pairs(all_unittypes) do
		if not pyr_npt.helper.tablecontains(chosen_units, k) then
...
function unit_selection.do_selection(side, recent_picks)
	...
	state.recent_picks = pyr_npt.helper.arrayfilter(recent_picks, has_unit)
...
function unit_selection.show_dialog(state)
	...
	local function preshow(dialog)
		local d_unit_list = dialog:find("unit_list")
		local d_chosen_unit_list = dialog:find("chosen_unit_list")
		local d_race_list = dialog:find("race_list")
		local d_unit_preview = dialog:find("unit_preview")
		...
		local on_selected_race_changed = function(widget)
			...
			for index2, value in ipairs(state.unit_types) do
				if value.race == state.race_id or (state.race_id == "pyr_npt_recently_selected" and pyr_npt.helper.tablecontains(state.recent_picks, value.id)) then
		...
		local update_chosen_units = function()
			...
			while index <= max_selectalbe_units_l do
				d_chosen_unit_list:find(index, "list_image2").label = pyr_npt.helper.thex_png
				index = index + 1
			end
			...
			if(maxcost == 1000000000) then
				-- disabled
				-- _"Cost:" ..  tostring(costs) .. cost_spaces_to_add
				dialog:find("pyr_total_cost").marked_up_text = ""
			else
				dialog:find("pyr_total_cost").marked_up_text = _"Cost:" ..  tostring(costs) .. "/"..  tostring(maxcost) .. cost_spaces_to_add
			end
		end
		...
		dialog.random_choice_button.on_button_click = on_random_button
