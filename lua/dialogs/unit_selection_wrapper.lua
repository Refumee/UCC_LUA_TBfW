...
function unit_selection_wrapper.let_player_choose_sides()
	...
	local result = wesnoth.sync.evaluate_multiple(
		...
		function(side)
			if wesnoth.sides[side].controller == "human" then
				local recent_picks = pyr_npt.helper.comma_to_list(global_vars.recent_picks)
				while true do
					local retv = pyr_npt.unit_selection.do_selection(side, recent_picks)
					if pyr_npt.unit_confirmation.confirm_recruitlist(retv) then
						local new_recent_picks = pyr_npt.helper.deepcopy(retv)
	...
	for k,v in pairs(result) do
		...
		if v.recruits ~= nil then
			...
			local should_ignore_invalid = (side.controller == "ai" or side.controller == "null")
	...
	wml.variables.pyr_npt_has_chosen_unit = true
end
