...
_G.ucc_open_picker = function(unit)
	...
	local unit_type_check = dialog_result.ut
	-- removes the ut value from the table
	dialog_result.ut = nil
	...
	if final_selections then
		...
		local safe_unit_type = string.gsub(unit.type, " ", "_")
		
		if whole_faction and not unit_type_check then
			for part, variant in pairs(final_selections) do
				wml.variables["ucc_pref_side_" .. unit.side .. "_" .. part] = variant
			end
			units_to_process = wesnoth.units.find_on_map({ side = unit.side })

		elseif whole_faction and unit_type_check then
			for part, variant in pairs(final_selections) do
				wml.variables["ucc_pref_side_" .. unit.side .. "_type_" .. safe_unit_type .. "_" .. part] = variant
			end
			units_to_process = wesnoth.units.find_on_map({ side = unit.side, type = unit.type })

		elseif unit_type_check and not whole_faction then
			units_to_process = wesnoth.units.find_on_map({ side = unit.side, type = unit.type })
...
on_event("unit placed", function(ctx)
	...
	for _, part in ipairs(race_data.body_parts) do
		local type_var_name = "ucc_pref_side_" .. unit.side .. "_type_" .. safe_unit_type .. "_" .. part.name
		local general_var_name = "ucc_pref_side_" .. unit.side .. "_" .. part.name
		
		local saved_variant = wml.variables[type_var_name] or wml.variables[general_var_name]
